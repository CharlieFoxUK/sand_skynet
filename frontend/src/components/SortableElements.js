...
class SortableElements extends Component {
    constructor(props) {
        super(props);
        // Deep clone the list to prevent "object is not extensible" errors from react-sortablejs
        const clonedList = this.props.list ? this.props.list.map(item => ({ ...item })) : [];
        this.state = {
            list: clonedList,
            showChildCross: true
        };
        this.lastList = clonedList;
    }
    ...
    componentDidUpdate() {
        if (!listsAreEqual(this.props.list, this.state.list)) {
            ...
            const clonedList = this.props.list ? this.props.list.map(item => ({ ...item })) : [];
            this.setState({ ...this.state, list: clonedList });
        }
    }
